...
export async function GET() {
  ...
  const lines: string[] = [
    ...
    'exists. A new revision is written when the dish changes. A stored',
    'revision is corrected only when the record of it is wrong.',
    '',
    'A dish can also go a different way. That is a variation: a recipe of',
    'its own, with its own address and its own revisions, beside the dish it',
    'came from rather than in place of it. A recipe says which dish it',
    'varies, and lists the whole family.',
    '',
  ...
  lines.push(
    ...
    `- [Science](${site.url}/science): the science note of each recipe, in one place. A note tells you what happens in the food, and why.`,
